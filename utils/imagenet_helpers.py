'''
This code is copied from https://github.com/MadryLab/robustness/blob/4033befe273b29f7b6dc36c30aa40696ed8fae96/robustness/tools/imagenet_helpers.py
and it has been modified to not require a training dataset
'''
...
class ImageNetHierarchy():
    ...
    '''
    def __init__(self, ds_info_path):
        """
        Args:
        ...
        ret = self.load_imagenet_info(ds_info_path)
        self.wnid_to_name, self.wnid_to_num, self.num_to_name = ret

        # ADDED
        self.in_wnids = self.wnid_to_num.keys()
    ...
    @staticmethod
    def load_imagenet_info(ds_info_path):
        """
        ...
        return wnid_to_name, wnid_to_num, num_to_name
    ...
    def get_superclasses(self, n_superclasses, 
        ...
        for (wnid, ndesc_in, ndesc_all) in self.wnid_sorted:
            ...
            if ancestor_wnid is None or self.is_ancestor(ancestor_wnid, wnid):
                ...
                # print('BEFORE:', [self.wnid_to_name[a] for a, b in superclass_info])
                ...
                # print('AFTER:', [self.wnid_to_name[a] for a, b in superclass_info])
    ...
    # ADDED
    def get_superclasses_by_min_size(self, min_size, exclude_if_less_than=0, print_excluded_nodes=False):
        """
        Whereas get_superclasses() requires the user to prespecify the number of desired superclasses,
        get_superclasses_by_min_count() simply requires the user to prespecify the minimum number of
        classes that should belong to each superclass. This method then returns as many (non-overlapping)
        superclasses of this size as possible.

        Algorithm: Start at root node of hierarchy (n00001740, "entity") then traverse down each branch until
        any of the children of the current node have fewer than min_size descendants. If this
        stopping criterion is met, add the current node to the list of superclasses to be returned.

        Args:
            - min_size (int): minimum number of classes that should belong in each superclass
            - exclude_if_less_than (int): If excluding one child with fewer than exclude_if_less_than 
            descendants (including itself) means that we can can continue descending down a branch with
            more than exclude_if_less_than descendants (including itself), we will exclude the classes
            in that branch
        """

        def get_superclasses_by_min_size_helper(min_size, curr_node, superclass_list, print_excluded_nodes=False):
            # print('CURR NODE', curr_node)
            descend = False
            for child in curr_node.children:
                subtree_sizes = np.array([c.descendant_count_in + 1 for c in child.children])
                if np.sum(subtree_sizes > exclude_if_less_than) >= 2:
                    descend = True
           
            # If all children have enough descendants or if one subtree has enough
            # children to justify throwing away a smaller subtree, then split 
            if descend:
        ...
        temp = list(set([x.name for x in superclass_list]))
        superclass_wnid = list(set([x.wnid for x in superclass_list])) # Convert to set to account for some classes appearing twice in tree
        
        class_ranges, label_map = self.get_subclasses(superclass_wnid, balanced=False)
